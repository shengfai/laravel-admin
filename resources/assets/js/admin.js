...
$(function () {
    ...
    $.form = new function () {
        ...
        // 加载HTML到弹出层
        this.modal = function (url, data, title, callback, loading, tips, width=800) {
            this.load(url, data, 'GET', function (res) {
                if (typeof (res) === 'object') {
                    return $.msg.auto(res);
                }
                var layerIndex = layer.open({
                    type: 1, btn: false, area: width + "px", content: res, title: title || '', success: function (dom, index) {
    ...
    $body.on('click', '[data-update]', function () {
        ...
        var tips = $(this).attr('data-tips') || '确定要操作这些数据吗？';
        $.msg.confirm(tips, function () {
            $.form.load(action, {field: field, value: value, id: id, _token: token}, 'patch');
        });
    });
    ...
    $body.on('click', '[data-delete]', function () {
        ...
        $.msg.confirm('确定要操作这些数据吗？', function () {
            $.form.load(action, {id: id, _token: token}, 'delete');
        });
    });
    ...
    $body.on('click', '[data-file]', function () {
        ...
        var url = window.ROOT_URL + 'plugs/upfile.html?mode=' + method + '&uptype=' + uptype + '&type=' + type + '&field=' + field;
    ...
    $body.on('change', '.pagination-trigger select', function () {
        $.form.open(this.options[this.selectedIndex].getAttribute('data-url'));
    });

    function ajaxRender(url,data= {},type){
        $.ajax({
            type: type || 'GET',
            url: url,
            data: data,
