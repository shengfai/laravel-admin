...
<div class="framework-topbar" style="position: fixed;top: 0;right: 0;width: 100%; " >
    ...
    @if ($menu['parent_id']==0)
    <a data-menu-node="m-{{ $menu['id'] }}" data-open="{{ $menu['url'] }}" class="topbar-btn pull-left transition">
        @empty(!$menu['icon'])<span class='font-s13 {{ $menu['icon'] }}'></span>&nbsp;@endempty
        {{ $menu['name'] }}
    </a>
    @endif
    @endforeach
    ...
    <div class="layui-tab" lay-unauto="" lay-allowclose="true" lay-filter="layadmin-layout-tabs">
        <ul class="layui-tab-title" id="LAY_app_tabsheader">
            <li lay-id="dashboards" lay-attr="dashboards" class="">
                <a data-open="{{ route('admin.dashboards.index') }}"><i class="layui-icon layui-icon-home"></i></a>
            </li>
        </ul>
    </div>
</div>
